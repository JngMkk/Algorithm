...
def parent(table: List[int], x: int) -> int:
    if table[x] != x:
        table[x] = parent(table, table[x])
    return table[x]


def union_(table: List[int], x: int, y: int) -> None:
    x = parent(table, x)
    y = parent(table, y)
    if x < y:
        table[y] = x
...
def solution(cards: List[int]) -> int:
    n: int = len(cards)
    table: List[int] = [x for x in range(n)]
    ...
    num: List[int] = sorted(Counter(table).values(), reverse=True)
    if len(num) == 1:
        return 0
    return num[0] * num[1]
