...
# 가장 긴 가로와 인접한 세로 길이를 빼면 빼야 할 세로 길이 나옴
# 가장 긴 세로와 인접한 가로 길이를 빼면 빼야 할 가로 길이 나옴
import sys

inp = sys.stdin.readline

k = int(inp())
direc = [list(map(int, inp().split())) for _ in range(6)]

max_w = 0
max_h = 0

for i in range(6):
    if direc[i][0] == 3 or direc[i][0] == 4:
        new_max = max(max_h, direc[i][1])
        if max_h != new_max:
            max_h = new_max
            max_h_ind = i

    else:
        new_max = max(max_w, direc[i][1])
        if max_w != new_max:
            max_w = new_max
            max_w_ind = i

minus = 1
for ind in [max_w_ind, max_h_ind]:
    left = ind - 1
    right = ind + 1
    if right == 6:
        right = 0
    minus *= abs(direc[left][1] - direc[right][1])

print((max_h * max_w - minus) * k)
